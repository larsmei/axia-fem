...
import type { Mesh, MeshElement, MeshNode, MeshQuality, Shape, Vec2 } from "./types";
...
function meshPolygonShape(shape: Shape, size: number, id0: { n: number; e: number }): Mesh {
  ...
  const nodes: MeshNode[] = pts.map((p, i) => ({ id: id0.n + i, x: p.x, y: p.y }));
...
function meshCirclePolar(shape: Extract<Shape, { kind: "circle" }>, size: number, id0: { n: number; e: number }): Mesh {
  ...
  const nodes: MeshNode[] = [{ id: id0.n, x: shape.cx, y: shape.cy }];
  for (let k = 1; k <= nr; k++) {
    const rr = (r * k) / nr;
    for (let t = 0; t < nt; t++) {
...
export function meshShapes(shapes: Shape[], size: number): Mesh {
  const id0 = { n: 1, e: 0 };
  const nodes: MeshNode[] = [];
  const elements: MeshElement[] = [];
  for (const s of shapes) {
    let part: Mesh;
    if (s.kind === "rect" && (!s.holes || s.holes.length === 0)) part = meshRectStructured(s, size, id0);
    else if (s.kind === "circle") part = meshCirclePolar(s, size, id0);
    else part = meshPolygonShape(s, size, id0);
...
function cornerAngles(pts: P[]): number[] {
  const n = pts.length;
  const out: number[] = [];
  for (let i = 0; i < n; i++) {
    const a = pts[(i + n - 1) % n];
...
export function meshQuality(mesh: Mesh): MeshQuality {
  ...
  for (const el of mesh.elements) {
    ...
    const aspect = Math.max(...edges) / Math.max(Math.min(...edges), 1e-12);
    const amin = Math.min(...angs, 180);
    minAngle = Math.min(minAngle, amin);
    maxAspect = Math.max(maxAspect, aspect);
    if (amin < 12 || aspect > 8) nBad += 1;
  }
  return {
