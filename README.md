...
**Repo:** [larsmei/axia-fem](https://github.com/larsmei/axia-fem) · **Releases:** [v1.31.9](https://github.com/larsmei/axia-fem/releases/tag/v1.31.9)

## Downloads (CLI 1.31.9)

Fertige Binaries am Release. Archiv enthält `axia` / `axia.exe`, `README`, `LICENSE` und `examples/`.

| Plattform | Datei |
|---|---|
| Windows x64 | [axia-1.31.9-windows-x64.zip](https://github.com/larsmei/axia-fem/releases/download/v1.31.9/axia-1.31.9-windows-x64.zip) |
| Linux x64 (glibc) | [axia-1.31.9-linux-x64.tar.gz](https://github.com/larsmei/axia-fem/releases/download/v1.31.9/axia-1.31.9-linux-x64.tar.gz) |
| Linux x64 (musl, statisch) | [axia-1.31.9-linux-x64-musl.tar.gz](https://github.com/larsmei/axia-fem/releases/download/v1.31.9/axia-1.31.9-linux-x64-musl.tar.gz) |
| Linux ARM64 | [axia-1.31.9-linux-arm64.tar.gz](https://github.com/larsmei/axia-fem/releases/download/v1.31.9/axia-1.31.9-linux-arm64.tar.gz) |
| macOS Apple Silicon | [axia-1.31.9-macos-arm64.tar.gz](https://github.com/larsmei/axia-fem/releases/download/v1.31.9/axia-1.31.9-macos-arm64.tar.gz) |
| macOS Intel | [axia-1.31.9-macos-x64.tar.gz](https://github.com/larsmei/axia-fem/releases/download/v1.31.9/axia-1.31.9-macos-x64.tar.gz) |

Prüfsummen: [SHA256SUMS](https://github.com/larsmei/axia-fem/releases/download/v1.31.9/SHA256SUMS). Standard-Solver ist **faer** (kein Intel-MKL nötig).

**1.31.9** — MYSTRAN-Validierung ([larsmei/mystran-validation](https://github.com/larsmei/mystran-validation)): acht Referenzfälle innerhalb rtol \(5\cdot10^{-5}\). `FORCE`/`MOMENT`/`GRAV` sind `SCALEF` mal die Komponenten, ohne Normierung des Richtungsvektors (MYSTRAN, nicht MSC). Nastran-Exponenten `1.+7` und `10.+06`. `CBUSH`: leeres CID entlang GA–GB, CID 0 ist das Basissystem; ohne Fortsetzung sitzt die Feder bei \(S=0{,}5\). `BAROR` liest die Orientierung in den Feldern 6–8. Torsion mit `J=0` hält `AUTOSPC`; Stiftmechanismen und MPC-Master bleiben frei. `RBE3` mit leerem Feld 2, abhängige Drehungen aus den Hebelarmen. Die F06 nennt `OUTPUT FOR SUBCASE` und Eigenwerte als \(\lambda\), rad/s und Hz. **1.31.8** — Offizielle ccx-2.23-Fälle: `*EXPANSION,ZERO` setzt Tref (beam8t |u| exakt). `CENTRIF` ist ω², Achspunkt und Achsrichtung (achtelc |u| exakt); CAX integriert ρ ω² r an den Gaußpunkten (axial |u| +0,02 %). `*RADIATE` gegen eine Senkentemperatur im stationären Newton, Anfangstemperatur ist kein Dirichlet (oneel8ra, Fläche −0,9507). `*NODAL THICKNESS` an S8 mit h(ξ)³ und konsistenter Masse (shellf f1 −2,9 %). Statische Vorspannung vor `*FREQUENCY` geht als Kg(σ) ein (beamf2 nahe der Knicklast, f2 −1 %). **1.31.7** — Analytische CalculiX-Fälle: `*DLOAD` auf C3D8/C3D20 drückt bei positivem Druck (wie ccx), Temperaturspannung zieht αΔT ab (σ=−EαΔT), zweispaltige `*BOUNDARY`-Karten bleiben bei ihrem Knoten, B32-Masse ist die konsistente Zeilensumme (Kragträger f1=831 Hz statt 786 Hz). **1.31.6** — `*STATIC, RIKS`: die FRD enthält jedes konvergierte Inkrement (`*NODE FILE`/`*EL FILE`), nicht nur den Endzustand. Die Lastfallzeit ist λ, auch negativ auf dem Rückweg (Mecway-Bogen: 66 Lastfälle). **1.31.5** — Mecway-Riks (`B32R` + `T3D3`, Amplitude −400 N): Knotenfolge Ende–Mitte–Ende, der T3D3-Mittelknoten bleibt per MPC auf der Sehne. Nach einem leichten Inkrement geht die Bogenlänge wieder auf das maximale Δλ; sonst bleibt ein Cutback am Limitpunkt hängen (λ=0,83 nach 100 Inkrementen statt λ=1). Der Bogen schnappt durch (|u|≈2,23 mm). `*STEP, INC` gilt, wenn `*STATIC` keine fünfte Zahl hat. **1.31.4** — `*STATIC, RIKS` im Browser: die Tangente am Limitpunkt ist indefinit, Cholesky (WASM, ohne faer) ist dort abgebrochen (`Inkrement 8`, \(\lambda\approx 0{,}40\)). Dichte LU übernimmt, das von-Mises-Fachwerk schnappt durch (\(\lambda=1\), Apex unter der Basis). **1.31.3** — NLGEOM, `C3D20`: Cauchy an den 27 Gaußpunkten, Lagrange auf die Knoten, wie die lineare Rechnung. Vorher lag der Elementmittelwert auf allen 20 Knoten und die Biegung hob sich auf (Winkeleisen \(\sigma_\mathrm{vm}\) max 44 MPa statt 468 MPa; linear 459 MPa). **1.31.2** — NLGEOM ohne Kontakt und ohne Pretension nimmt den vollen Newton-Schritt, auch wenn das Residuum nach der ersten Korrektur durch die Green-Lagrange-Kraft um eine Größenordnung steigt. Halbieren bis das Residuum fällt verwirft diesen Schritt (Winkeleisen, 72×C3D20, eine Laststufe: 5 Iterationen statt 24 und einem Cutback). Kontakt und Pretension halbieren weiter; die Verschiebungskappe gilt nur dort. **1.31.1** — Mecway-BDF: `CQUAD8` als S8 (Membran \(u=FL/EA\)). `CHEXA` mit 20 Knoten nimmt auch die Mecway-Reihenfolge der Mittenknoten. **1.31.0** — MYSTRAN Phase 8: jede Bulk-Karte aus Kapitel 11 des Manuals 2025-09-22 ist gerechnet oder abgewiesen. Fremde Karten (zum Beispiel `PLOAD1`) sind ein Fehler mit dem Namen. `SOL 31` (Craig-Bampton) und `SOL 4/104` (Differentialsteifigkeit) werden nicht als Statik gerechnet. `PLOTEL`, `SEQGP` und `SPOINT` ohne Wirkung. **1.30.0** — MYSTRAN Phase 7: F06 schreibt für ROD Normalkraft und Axialspannung (Zug positiv) und für CBAR die Schnittgrößen M1a, M2a, M1b, M2b, V1, V2, Axialkraft und Torsion. OP2 `OUGV1` neben der F06. CROD ohne Torsion. Balken-Spannungspunkte C/D/E/F werden nicht angesetzt. **1.29.0** — MYSTRAN Phase 6: `SOL 5/105` lineares Beulen, `EIGR` neben `EIGRL`, alle Eigenvektoren in der F06. `PARAM` `AUTOSPC` (Vorgabe an; `NO` lässt lose Freiheitsgrade frei), `K6ROT` skaliert die MITC4-Drilling-Strafe, `GRDPNT` schreibt die Starrkörpermasse. **1.28.0** — MYSTRAN Phase 5: `MAT2`, `MAT8`, `MAT9` und `PCOMP`/`PCOMP1`. `CQUAD4`/`CTRIA3` werten `THETA` und `MCID` aus (90° tauscht \(E_1\) und \(E_2\); nicht MYSTRAN Issue 102). `MAT9` nur auf linearem `CHEXA`. `PCOMP`-Kopplung \(B\) wird gemeldet und nicht angesetzt, `NSM` geht in die Dichte. Versagenskriterien ohne Wirkung. **1.27.0** — MYSTRAN Phase 4: `RFORCE` mit \(\omega=2\pi V\) (Umdrehungen), `TEMP`/`TEMPD`/`TEMPP1`/`TEMPRB` und `MAT1` TREF (Dehnung \(\alpha(T-T_\mathrm{ref})\)), erzwungene `SPC`-Verschiebung. `PLOAD1` steht nicht im Manual 2025-09-22. Gradienten `TPRIME`/`TP*` werden abgewiesen, nicht still ignoriert. **1.26.0** — MYSTRAN Phase 3: `CBAR` als Hermite-Balken mit Schub (Kragarm-Spitze \(3EI/L^3\); CalculiX-B31 bleibt \(4EI/L^3\)), `BAROR`, `PBARL` ROD/TUBE/TUBE2/BAR/BOX/I/T/L, Stiftfahnen PA/PB, Endversatz. `CELAS1`–`4`/`PELAS` als Komponentenfedern, `CMASS1`–`4`/`PMASS`, `CONM2` mit Versatz und Drehträgheit. `PSHELL`-Feld `12I/T³`, Aliase `CQUAD4K`/`CTRIA3K`, `CSHEAR`/`PSHEAR`, `CBUSH`/`PBUSH` (K). Eine singuläre Steifigkeit wird abgewiesen. **1.25.0** — MYSTRAN Phase 2: `CORD1/2` rechteckig, zylindrisch und sphärisch, `GRID CD` in der F06, `MPC`/`RBE3`, `RBE2` mit Teil-CM. `ASET`/`OMIT` werden abgewiesen. **1.24.0** — MYSTRAN Phase 1: Großfeld (`GRID*`), Fortsetzung in Spalte 73, freie Fortsetzung mit Komma, `THRU`, `INCLUDE` und `ECHO` ändern den CROD nicht. Eine unterstützte Karte ohne Pflichtfeld ist ein Fehler. **1.23.0** — MYSTRAN Phase 0: Kartenmanifest (User Manual Kapitel 7), Golden `FL/EA` für den CROD, harte Ablehnung von `CUSERIN`/`PARVEC`. `PLOAD4` in `LOAD` wird gerechnet und kopiert nicht MYSTRAN Issue 196. BDF/F06-Basis: SOL 1/101 und 3/103, Stab, Balken, Schale, Volumen, SPC und Lasten. **1.22.0** — S4/S4R als gekrümmte MITC4 (Dvorkin–Bathe): bilineare Mittelfläche, flächengewichtete Knotendirektoren, Membran und Krümmung in der Flächenmetrik, Querschub an den Kantenmitten gebunden. Drilling-Strafe \(10^{-8}EhA\) statt \(10^{-4}\), sonst sperrt sie den inextensionalen Mode dünner Schalen. NAFEMS LE3 (Halbkugel, 18°-Ausschnitt): \(u_x(A)=0{,}183/0{,}184/0{,}185\,\mathrm{m}\) bei 4×4/8×8/16×16 (Ziel 0,185; Bathe-MITC4 104/101/100 %). **1.21.0** — Quadratischer Node-to-Surface-Kontakt: C3D20-Flächen mit allen 8 Knoten (Ecken + Mittelknoten) statt nur Ecken; Master in 6 Dreiecke; positive Lumpung (Ecken \(A/12\), Mittelknoten \(A/6\)); Penalty-Steigung \(k_n A\) ohne nodal 10¹⁰-Kappe (sonst \(k_n=2\cdot10^{14}\) flach). Newton: Uphill-Accept erst ab Iteration 2. Schraubenverbindung Dummy ~44 µm. **1.20.0** — Kontakt+NLGEOM: `*STEP,AMPLITUDE=STEP` setzt den Pretension-Dummy (30 kN) voll ab Inkrement 1, die Querkraft ramppt weiter; SYZ ist nicht mehr \(t\)-linear. Cauchy-Rückrechnung (C3D20, 27 GP, Lagrange) und Kontakt auf deformierter Geometrie. Newton: Inkrement 1 Vollstick, danach Coulomb; voller erster Schritt, Line-Search danach, Uphill-/Plateau-Abbruch. Schraubenverbindung: 10 DIRECT-Inkremente, Dummy ~69 µm. **1.19.0** — PARDISO/MKL unter Linux und macOS: `pardiso-wrapper` findet Libraries nur mit Execute-Bit (`which`); Distro-`.so`/`.dylib` sind oft 644. Axia macht sie sichtbar (`chmod +x` oder Shim) und sucht oneAPI, Distro, Conda, Homebrew und `ldconfig`. Fehlt MKL, stehen Installationsschritte auf stderr. **1.18.0** — Kontakt bei Pretension: glattes Coulomb \(f_t=\tau g_t/\sqrt{|g_t|^2+g_\mathrm{char}^2}\) (\(\tau=\mu p + k_{t,\mathrm{floor}} g_\mathrm{char}\)) statt Vollstick auf dem ganzen Lap; SI_19-Kopf reibungsfrei wie CalculiX (kein `kt_stab` bei \(\mu=0\)); Newton mit kleineren Line-Search-Alphas und Plateau-Abbruch. Schraubenverbindung 30 kN: Residuum ~2 N, 6 Gleitknoten, Dummy ~69 µm. **1.17.0** — FRD wie CalculiX 2.22 / Mecway: `1PSTEP` unmittelbar vor jedem `100CL`, Kode `100+iinc` (nicht 101/102/103 als Datensatztyp), leeres Namensfeld, Reihenfolge DISP→STRESS→FORC, Pretension-Dummy und MASS-only-Knoten weggelassen (`nout=730`). **1.16.0** — FRD schreibt jedes `*STATIC,DIRECT`-Inkrement (DISP/STRESS/FORC, t=0.1…1.0 wie CalculiX). C3D20-Spannungen aus 27 Gaußpunkten mit Lagrange-Extrapolation statt B-Matrix an den Knoten (Schraubenverbindung: vmMax 1.08 GPa statt 1.76, Mittel 152 vs. ccx 142 MPa). **1.15.0** — `*PRE-TENSION SECTION` wie CalculiX: Copy-Knoten in den Surface-Elementen (`gen3delem`), Dummy-MPC \(\mathbf{n}\cdot(\mathbf{u}_\mathrm{copy}-\mathbf{u}_\mathrm{orig})=u_\mathrm{dummy}\), Dummy-Hold auf Schur-\(K_{dd}\) statt Jacobi (Schraubenverbindung 30 kN: Dummy-Öffnung ~67 µm, Residuum ~20 N). **1.14.0** — `*PRE-TENSION SECTION` (Schnittöffnung, Dummy-MPC, 30 kN-Schraubenverbindung), ccx-Statuszeilen während NLGEOM (Inkrement/Attempt/Iteration/Residuum), `*TIE` mit Default-Toleranz, Kontakt-Penalty nach `*SURFACE INTERACTION` auch wenn das Paar davor steht, AMPLITUDE=STEP. Beispiel `bolted_joint.inp`. **1.13.0** — NLGEOM-Erweiterung: 2D-Kontinuum (CPS/CPE/CAX) Total-Lagrange, Schalen/Balken korotational, Hyperelastizität (Neo-Hooke/Ogden/Hyperfoam), starre Finite-Rotation (`*RIGID BODY` im Newton), Kontakt auf deformierter Geometrie, Folgelast (Druck auf aktueller Fläche), geklebte Keywords `*SOLIDSECTION`/`*RIGIDBODY`. Offizieller Test `scheibe` (180°-Starrkörper). **1.12.0** — NLGEOM-Korrektur: `*STATIC` als Δt/Periode (nicht Inkrementzahl), Lastinkremente mit Δt-Cutback statt linearem Fallback bei Inversion, Mehrschritt-Fortsetzung von `u`/`f`, `OP=NEW` auf `*BOUNDARY`/`*CLOAD`/`*DLOAD`, plastische Gauss-Historie nach konvergiertem Inkrement, Amplituden am Periodenende. Kontinuum-SVK (`beamnld`/`beamnlp`) matcht ccx. **1.11.0** — Präprozessor 3D (Quader/Zylinder/Kugel, C3D8/C3D6, Flächenlager, Orbit-Viewport) und breitere CalculiX-INP-Kompatibilität (Fortran-`1.d0`, verschachtelte Sets, `CENTRIF`, `TYPE=ORTHO`/`ENGINEERING`, `BEAM SECTION=BOX`, `TRANSFORM TYPE=C`, Composite-Schalen, `ROT NODE`, lineare Fallbacks für NLGEOM/Kontakt). Vergleichsläufer `solver/scripts/ccx-compare.py` gegen die ccx-2.23-Suite. **1.10.0** — Browser-Präprozessor: Rechteck/Kreis/Polygon, strukturiertes Quad-Netz und Delaunay, Stahl/Alu/Beton, Kantenlager und Kräfte, Modellcheck, CalculiX-INP (CPS3/CPS4), Übergabe an den WASM-Solver. Vorlagen Kragarm, gelochte Platte, Zugstab. **1.9.8** — `*TIE`/`*COUPLING`/`*CONTACT` auf C3D20/C3D10/C3D15-Flächen inkl. Mittelknoten (vorher nur Ecken → klaffende Kanten). **1.9.7** — MKL: kein stiller Prozessabbruch mehr. Unvollständige Redist (ohne `mkl_core` / CPU-Kernel `mkl_avx2`/`mkl_def`) wird vor `pardiso()` erkannt; Selbsttest in einem Kindprozess fängt Intel-`abort()` (FATAL ERROR / OpenMP #15). `--solver auto` fällt dann auf **faer** zurück, `--solver mkl` schreibt die Ursache auf stderr. `KMP_DUPLICATE_LIB_OK=TRUE`, `perm[n]`, Matrix-Checker. **1.9.6** — FRD: C3D20/C3D20R und C3D15 mit ccx-Midside-Permutation (Abaqus-INP → FAM/he20), sonst verdrehtes Hex20 in Mecway/cgx. **1.9.5** — FRD: `MASS`/`ROTARYI` wie ccx nicht ins Elementnetz (Mecway-Typ 11 braucht zwei I10-Felder; sonst „Field of length 10 missing at column 14“). **1.9.4** — Sparse-Solver per `--solver` / `AXIA_SOLVER`: **faer** (reines Rust, supernodales \(LL^\top\)/\(LU\), keine extra Library, PARDISO-Klasse für SPD). `auto` = MKL → Panua → faer → rivrs-sparse. **1.9.3** — Mecway/CalculiX: `*ELEMENT` ohne `ELSET=` bekommt Material/Dicke/`*MASS` über spätere `*ELSET`-Mitgliedschaft; MASS/DASHPOT/GAP brauchen kein Kontinuum-`*MATERIAL`; GRAV auf `*MASS` als \(F=mg\). **1.9.2** — Windows-MKL: `mkl_rt.dll` **neben `axia.exe`** (Shim `libmkl_rt.dll` im selben Ordner, nicht in `%TEMP%`). **1.9.1** — FRD long-ASCII wie ccx (Spalte 74, Fortran `E-02`, ` -1`/` -2`/` -3`), Windows-MKL findet `mkl_rt.dll` unter `$MKLROOT/bin`, Viewer Jet-Farbverlauf / Mecway-Colorbar. **1.9** — fehlende Elemente: CAX3/CAX6, C3D10T (B-bar), MASS, ROTARYI, DASHPOTA, GAPUNI, Aliase B21/B22 und C3D20RI. **1.8** — `*STATIC, RIKS` (Crisfield-Bogenlänge, Snap-Through). **1.7.1** — Viewer zeichnet T3D2/T3D3/SPRINGA (Fachwerk NLGEOM). **1.7** — Coulomb-`*FRICTION`, `NLGEOM`+`*PLASTIC` auf Kontinuum. **1.6** — `*CONTACT PAIR` Node-to-Surface, Penalty, reibungsfrei. **1.5** — `*PLASTIC` J2 für Kontinuum (C3D*), PEEQ im FRD. **1.4** — `*STEP, NLGEOM` für Kontinuum (C3D8/20/4/10/6/15), Total-Lagrange St. Venant–Kirchhoff. **1.3** — mehrere `*STEP`, `*CONTROLS`. **1.2** — C3D15, CAX, Membran, Kontinuum-Beulen, Wärme+. **1.1** — Wärme, Dynamik, NLGEOM/`*PLASTIC` (T3D2).
...
Wie CalculiX: Job-Name ohne Endung. `axia job` liest `job.inp` und schreibt `job.frd` plus `job.dat`. Ein MYSTRAN-Deck (`job.bdf`, auch `.nas`/`.dat`) schreibt stattdessen `job.f06`. `--format calculix` oder `--format mystran` wählt das Ausgabeformat unabhängig von der Eingabe.
...
axia examples/mystran_rod.bdf          # → mystran_rod.f06
axia --format calculix model.bdf       # BDF rechnen, FRD/DAT schreiben
```

## MYSTRAN / Nastran-BDF

Ein Deck mit `BEGIN BULK` / `ENDDATA` (oder eine Bulk-Datei, die mit `GRID` beginnt) wird als MYSTRAN gelesen. Reelle Felder akzeptieren Nastran-Exponenten (`1.0+3`, `1.+7`, `10.+06`). Ohne `--format` schreibt Axia dann eine F06-Datei; ein CalculiX-INP bleibt bei FRD/DAT. `--format calculix` oder `--format mystran` erzwingt das jeweils andere Ausgabeformat.

Unterstützt, klein- und freifeldfähig, mit `$`-Kommentar, Fortsetzungszeile und `INCLUDE`:

| Karte | Abbildung |
|---|---|
| `SOL 1/101` | linear-statisch |
| `SOL 5/105` | lineares Beulen, Faktoren und Moden in der F06 |
| `SOL 3/103` + `EIGRL`/`EIGR` | Eigenfrequenzen (Hz); jeder Eigenvektor in der F06 |
| `GRID`, `CORD1/2` R/C/S, `GRDSET` | Knoten; zylindrisch R/θ/Z, sphärisch R/θ/φ; `CD` ist das Ausgabesystem der F06 |
| `MPC`/`MPCADD`, `RBE2`, `RBE3` | Gleichungen, Starrkörper mit Teil-CM. `RBE3`: leeres Feld 2, danach REFGRID/REFC. Abhängige Drehungen folgen aus den Hebelarmen der unabhängigen Verschiebungen, nicht aus einer komponentenweisen Mittelung |
| `CROD`/`CONROD`/`PROD` | T3D2, Fläche über die Dicke-Ablage. F06: Normalkraft und Axialspannung, Zug positiv, Torsion 0 |
| `CBAR`/`CBEAM` + `PBAR`/`PBARL` (ROD, TUBE, TUBE2, BAR, BOX, I, T, L) + `BAROR` | Hermite-Balken mit Schub, Spitze \(3EI/L^3\). `BAROR` legt die Orientierung in die Felder 6–8. PA/PB-Stifte, Endversatz. `J=0` lässt die Torsion offen (`AUTOSPC` hält die Null-Diagonale). F06: M1a, M2a, M1b, M2b, V1, V2, Axial, Torsion. Orientierung ist lokale y; Nastran \(I_1\) biegt in der x-y-Ebene. CalculiX-B31 bleibt \(4EI/L^3\) |
| `CQUAD4`/`CQUAD4K`/`CQUAD8`/`CTRIA3`/`CTRIA3K` + `PSHELL` | S4/S3 (MITC4), S8. `12I/T³` skaliert die Biegung. `THETA`/`MCID`. `MID2=-1` wird Membran |
| `MAT2`/`MAT8`, `PCOMP`/`PCOMP1` | anisotrope und orthotrope Platte, Lagenaufbau. `Z0`-Vorgabe \(-t/2\) |
| `MAT9` + `CHEXA` | anisotropes 6×6, nur lineares Hexaeder |
| `CSHEAR` + `PSHEAR` | Schubfeld, nur γxy |
| `CBUSH` + `PBUSH` | diagonale Feder (`K`). Leeres CID: x entlang GA–GB, z = x×v. CID 0 ist das Basissystem. Ohne Fortsetzung sitzt die Feder bei \(S=0{,}5\) auf der Linie GA–GB |
| `CTETRA`/`CHEXA`/`CPENTA` + `PSOLID` | C3D4/10, C3D8/20, C3D6/15. `CHEXA`20 auch in Mecway-Mittenknotenreihenfolge |
| `CELAS1`–`4` + `PELAS`, `CMASS1`–`4` + `PMASS`, `CONM2` | Komponenten-Feder (auch geerdet), skalare Masse, Punktmasse mit Versatz und Drehträgheit |
| `SPC`/`SPC1`/`SPCADD`, `FORCE`/`MOMENT`, `LOAD` | Lager, erzwungene Verschiebung und Lastkombination. `FORCE`/`MOMENT` sind Betrag mal Komponenten, ohne Division durch die Vektorlänge |
| `RFORCE` | Starrkörperdrehung, \(\omega=2\pi V\), Fliehkraft nach außen |
| `TEMP`/`TEMPD`/`TEMPP1`/`TEMPRB` | Wärmedehnung \(\alpha(T-T_\mathrm{ref})\); Elementtemperatur geht vor Gitter |
| `PLOAD2`/`PLOAD4` | Schalendruck positiv entgegen der Elementnormalen; Volumen mit `G1`/`G34` |
| `GRAV`, `PARAM` | `GRAV` ist `SCALEF` mal Komponenten, nicht mal den Einheitsvektor. `WTMASS` skaliert die Dichte. `AUTOSPC` hält numerisch leere Diagonalen, nicht MPC-Master, Vorspannungs-Dummyknoten oder Stabgelenke. `K6ROT`, `GRDPNT` |
| `SUBCASE` | unabhängige Lastfälle, jeder Block in der F06 |

Die F06-Gittertabellen folgen MYSTRAN (`OUTPUT FOR SUBCASE`, `D I S P L A C E M E N T S`, `S P C   F O R C E S`, 108 Spalten, `ES14.6`). Eigenwerte stehen als \(\lambda=\omega^2\), rad/s und Hz. ROD und BAR schreiben Elementkräfte und Spannungen in MYSTRAN-Reihenfolge; andere Elemente stehen als Mittel der Knotenspannungen. Neben der F06 liegt eine OP2 mit `OUGV1`.

Vollständige Kompatibilität ist der [Fahrplan](solver/src/mystran_manifest.rs) in Phasen. Phase 8 (1.31.0) schließt Kapitel 11: unbekannte Bulk-Karten sowie `SOL 31` und `SOL 104` werden abgewiesen. Phase 7 (1.30.0) schreibt ROD- und BAR-Schnittgrößen in die F06 und die Verschiebungen als OP2 `OUGV1`. Phase 6 (1.29.0) rechnet `SOL 5/105`, schreibt alle Eigenvektoren und kennt `AUTOSPC`/`K6ROT`/`GRDPNT`. Phase 5 (1.28.0) rechnet `MAT2`/`MAT8`/`MAT9` und `PCOMP`/`PCOMP1`; der Materialwinkel wird angesetzt (Quadrat, \(E_2=3\), \(\theta=90^\circ\), Kraft 1 → \(u=1/3\)). Phase 4 (1.27.0) rechnet `RFORCE`, `TEMP`/`TEMPD`/`TEMPP1`/`TEMPRB` und erzwungene `SPC`. Phase 3 (1.26.0) rechnet `CBAR` als Hermite-Balken mit Schub (\(3EI/L^3\)), `BAROR`, die übrigen `PBARL`-Typen, Stifte, Versatz, `CELAS`/`CMASS`, `CONM2`-Trägheit, `PSHELL` `12I/T³`, `CSHEAR` und `CBUSH`. Phase 2 (1.25.0) liest `CORD1/2` R/C/S, `GRID CD`, `MPC` und `RBE2`/`RBE3`. `ASET`/`OMIT` werden abgewiesen. Phase 0 friert die Kartenliste ein: implementiert, geplant, abgelehnt oder ein dokumentierter MYSTRAN-Defekt. Der CROD-Golden liegt in `solver/tests/mystran/`. Toleranz gegen analytische Werte: relativ \(10^{-4}\), absolut \(10^{-8}\). CalculiX-B31 bleibt der Ein-Punkt-Timoshenko-Balken (\(4EI/L^3\)).

```bash
axia examples/mystran_rod
```
...
| `mystran_rod.bdf` | MYSTRAN/Nastran CROD, Ausgabe `.f06` und `.op2` |
...
| `bolted_joint.inp` | C3D20-Lasche, `*PRE-TENSION SECTION` 30 kN, Kontakt + TIE |
...
#### Intel MKL unter Linux

`pardiso-wrapper` sucht `libmkl_rt.so` mit der `which`-Logik — die Datei braucht das **Execute-Bit**. Distro-Pakete installieren `.so` oft als `644`, deshalb hat Axia bis 1.18 PARDISO unter Linux oft nicht gefunden. Ab jetzt: Axia macht die Library für den Wrapper sichtbar (`chmod +x` oder Kopie in einem Shim) und sucht zusätzlich in `/opt/intel/oneapi/mkl/latest`, `$CONDA_PREFIX`, `/usr/lib`, `/usr/lib/x86_64-linux-gnu`, Homebrew-Pfaden und `ldconfig -p`.

Einrichtung, eine davon:
...
# 2) Distro-Paket
sudo apt install intel-mkl            # Debian/Ubuntu
# sudo dnf install intel-oneapi-mkl   # Fedora/RHEL
# pacman -S intel-oneapi-mkl          # Arch

# 3) Conda
conda install -c https://software.repos.intel.com/python/conda mkl

# 4) Portable: Dateien neben das axia-Binary legen
...
`--solver mkl` ohne gefundene Library schreibt genau diese Schritte auf stderr. Ohne MKL: `axia --solver faer`.

#### Intel MKL unter macOS

Intel MKL PARDISO ist **nur x86_64**. Auf Apple Silicon gibt es kein MKL — `axia --solver faer` (oder Panua, falls vorhanden). Intel hat oneMKL für macOS nach **2023.2** eingestellt.

Falls 2023 noch installiert ist:
...
Sonst die dylibs neben `axia` legen: `libmkl_rt.dylib`, `libmkl_core.dylib`, `libmkl_sequential.dylib` (oder `libmkl_intel_thread.dylib` + `libiomp5.dylib`), plus `libmkl_avx2.dylib` oder `libmkl_def.dylib`.

Axia setzt `DYLD_LIBRARY_PATH` / `DYLD_FALLBACK_LIBRARY_PATH` und erzeugt bei Bedarf einen ausführbaren Shim (`libmkl_rt.dylib`), weil `pardiso-wrapper` sonst 644-dylibs übersieht. SIP kann `DYLD_LIBRARY_PATH` in Kindprozessen streichen — der Shim umgeht das.

#### Panua PARDISO

`libpardiso.so` / `libpardiso.dylib` / `libpardiso.dll` neben `axia`, auf `LD_LIBRARY_PATH` / `DYLD_LIBRARY_PATH`, oder `PARDISO_PATH` auf den Ordner. Download und Lizenz: [panua.ch/pardiso](https://panua.ch/pardiso/).
...
|---|---|---|
| S4 / S4R | 4 | gekrümmte MITC4 (Knotendirektor) — kein Schub-Locking |
...
| NAFEMS LE3 S4 | Halbkugel 18°-Loch, \(u_x(A)=0{,}185\,\mathrm{m}\) (4×4: 0,183; 8×8: 0,184) |
