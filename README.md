...
| `CBAR`/`CBEAM` + `PBAR`/`PBARL` (ROD, TUBE, BAR) | B31, Orientierung als lokale y-Achse. Nastran \(I_1\) biegt in der x-y-Ebene |
| `CQUAD4`/`CTRIA3` + `PSHELL` | S4/S3 (MITC4). `MID2=-1` wird Membran |
| `CTETRA`/`CHEXA`/`CPENTA` + `PSOLID` | C3D4/10, C3D8/20, C3D6/15 |
| `CELAS2`, `CONM2` | axiale Feder, Punktmasse |
...
Vollständige Kompatibilität ist der [Fahrplan](solver/src/mystran_manifest.rs) in Phasen. Phase 2 (1.25.0) liest `CORD1/2` R/C/S, `GRID CD`, `MPC` und `RBE2`/`RBE3`. `ASET`/`OMIT` werden abgewiesen. Phase 0 friert die Kartenliste ein: implementiert, geplant, abgelehnt oder ein dokumentierter MYSTRAN-Defekt. Der CROD-Golden liegt in `solver/tests/mystran/`. Toleranz gegen analytische Werte: relativ \(10^{-4}\), absolut \(10^{-8}\). `CBAR` hängt noch am Ein-Punkt-Timoshenko-Balken (\(4EI/L^3\), nicht \(3EI/L^3\)).
