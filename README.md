...
Ein Deck mit `BEGIN BULK` / `ENDDATA` (oder eine Bulk-Datei, die mit `GRID` beginnt) wird als MYSTRAN gelesen. Ohne `--format` schreibt Axia dann eine F06-Datei; ein CalculiX-INP bleibt bei FRD/DAT. `--format calculix` oder `--format mystran` erzwingt das jeweils andere Ausgabeformat.
...
| `MPC`/`MPCADD`, `RBE2`, `RBE3` | Gleichungen, Starrkörper mit Teil-CM, gewichtete Interpolation |
| `CROD`/`CONROD`/`PROD` | T3D2, Fläche über die Dicke-Ablage. F06: Normalkraft und Axialspannung, Zug positiv, Torsion 0 |
| `CBAR`/`CBEAM` + `PBAR`/`PBARL` (ROD, TUBE, TUBE2, BAR, BOX, I, T, L) + `BAROR` | Hermite-Balken mit Schub, Spitze \(3EI/L^3\). PA/PB-Stifte, Endversatz. F06: M1a, M2a, M1b, M2b, V1, V2, Axial, Torsion. Orientierung ist lokale y; Nastran \(I_1\) biegt in der x-y-Ebene. CalculiX-B31 bleibt \(4EI/L^3\) |
...
| `CBUSH` + `PBUSH` | diagonale Feder im Bush-System (`K`) |
...
| `SPC`/`SPC1`/`SPCADD`, `FORCE`/`MOMENT`, `LOAD` | Lager, erzwungene Verschiebung und Lastkombination |
...
| `GRAV`, `PARAM` | `WTMASS`, `AUTOSPC`, `K6ROT`, `GRDPNT` |
| `SUBCASE` | unabhängige Lastfälle, jeder Block in der F06 |

Die F06-Gittertabellen folgen MYSTRAN (`D I S P L A C E M E N T S`, `S P C   F O R C E S`, 108 Spalten, `ES14.6`). ROD und BAR schreiben Elementkräfte und Spannungen in MYSTRAN-Reihenfolge; andere Elemente stehen als Mittel der Knotenspannungen. Neben der F06 liegt eine OP2 mit `OUGV1`.
